...
class ccc extends BaseControl{
      ...
      public function __construct(){
            ...
            $this->save_path =ENTRANCE_ROOT.'res/attached/';
            $this->save_url =ABS_ENTRANCE_PATH.'res/attached/';
            $this->load('ccc');
      }
      #显示添加数据界面
      public function onAdd(){
             $this->view('ttt/ttt_add.php');
      }
      #显示修改数据界面
      public function onModify(){
             $pk='ppk';
             $pid=G($pk);
             if(is_numeric($pid)&&$row=$this->model->ccc->getById($pid)){
                    $data['row']=$row;
      ...
      public function onPage(){
             #条件示例
             #$where=G('cat')===false?'1':'cat='.$this->db->escape(G('cat'));
             $where=1;
             $data=$this->model->ccc->page(10,null,'pageCols',$where);
             $data['pk']='ppk';
      ...
      public function onCreate(){
             if($this->model->ccc->insert()){
                    ajaxEcho(200,'添加成功');
             }elseif($this->model->ccc->msg){
                    ajaxEcho(400,$this->model->ccc->msg);
             }else{
                    ajaxEcho(400,'添加失败');
             }
      }
      ...
      public function onUpdate(){
             if($this->model->ccc->update()!==false){
                    ajaxEcho(200,'修改成功');
             }elseif($this->model->ccc->msg){
                    ajaxEcho(400,$this->model->ccc->msg);
             }else{
      ...
      #删除多个记录
      public function onDels(){
            $pks=P('pks');
           if(is_array($pks)){
              echo $this->model->ccc->deleteByIds($pks)?getAjaxJson(400,'删除成功'):getAjaxJson(400,'删除失败');
           }else{
              ajaxEcho(400,'error data.');
           }
      }
      #富文本编辑器上传文件接口
      public function onUploadJson(){
      ...
      #富文本编辑器浏览图片接口
      public function onFileManagerJson(){
            //根目录路径，可以指定绝对路径，比如 /var/www/attached/
      ...
      #富文本编辑框保存远程图片接口
      public function onSaveRemoteImage(){
