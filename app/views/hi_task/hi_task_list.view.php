...
<body>
<fieldset>
    <legend>列表页&nbsp;&nbsp;[<a href="?hiTask.add">添加</a>]</legend>
<hr class="hr1"/>
<table cellpadding="0" cellspacing="0" width="90%">
    <tr><th>ID</th><th>文章标题</th><th>添加时间</th><th>状态</th><th>添加者</th><th>类型</th><th>操作</th></tr>
<?php foreach($items as $row){;?>
    <tr><td><label><input type="checkbox" value="<?php  echo $row['id'];?>" class="pk"/><?php  echo $row['id'];?></label></td><td><?php  echo $row['title'];?></td><td><?php  echo $row['time'];?></td><td><?php  echo $row['status'];?></td><td><?php  echo $row['user'];?></td><td><?php  echo $row['type'];?></td>    <td>[<a href="?hiTask.modify&id=<?php  echo $row[$pk];?>">修改</a>]</td>
    </tr>
<?php }//end foreach;?>
</table>
